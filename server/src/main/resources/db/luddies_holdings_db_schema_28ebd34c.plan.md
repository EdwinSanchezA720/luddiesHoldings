...
overview: Diseñar e implementar la base de datos MySQL del backend de Luddies Holdings con un esquema normalizado de 10 tablas (alcance estándar), entregar el diagrama ER, los scripts `create.sql` e `insert.sql` ubicados en `server/src/main/resources/db/`, y dejar listada la estructura MVC mínima de Spring Boot que consumirá esta base.
todos:
  - id: create-folder
    content: Crear la carpeta luddiesHoldings/server/src/main/resources/db/
    status: completed
  - id: write-create-sql
...
- **Decisiones confirmadas**:
  ...
  - Ubicación: **`luddiesHoldings/server/src/main/resources/db/`**.
...
- Los 3 archivos viven en `luddiesHoldings/server/src/main/resources/db/`.
