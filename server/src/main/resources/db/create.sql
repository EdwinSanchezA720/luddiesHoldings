-- =============================================================================
-- Luddies Holdings — Script de creación de base de datos (MySQL 8.x)
...
CREATE TABLE roles (
  ...
  COMMENT='Roles de aplicación (ADMIN, USER).';
...
CREATE TABLE users (
  ...
  COMMENT='Cuentas de usuario (clientes y administradores).';
...
CREATE TABLE categories (
  ...
  COMMENT='Categorías canónicas del catálogo (filtros del cliente).';
...
CREATE TABLE products (
  ...
  COMMENT='Productos del marketplace (textos ES/EN en columnas).';

-- -----------------------------------------------------------------------------
-- 5. product_categories (N:M)
-- -----------------------------------------------------------------------------
CREATE TABLE product_categories (
  ...
  COMMENT='Relación producto ↔ categorías (múltiples etiquetas por producto).';
...
CREATE TABLE carts (
  ...
  COMMENT='Carritos de compra por usuario.';
...
CREATE TABLE cart_items (
  ...
  COMMENT='Líneas del carrito (un producto una sola vez por carrito en el front).';
...
CREATE TABLE order_items (
  ...
  COMMENT='Detalle de líneas de la orden (snapshot de título y precio).';
...
CREATE TABLE payments (
  ...
  method ENUM('SIMULATED_CARD') NOT NULL DEFAULT 'SIMULATED_CARD',
  ...
  COMMENT='Pago asociado 1:1 a la orden (simulación de tarjeta).';
