-- Luddies Holdings — Database schema
-- MySQL 8.0+ | InnoDB | utf8mb4
-- Run this file before insert.sql
...
-- Catalog categories (match the frontend filters)
...
-- Marketplace products with bilingual content (ES/EN)
...
-- Product <-> category relationship (a product can belong to multiple categories)
...
-- Items inside a cart (each product can only appear once per cart)
...
-- Purchase orders
-- user_id is NULL when the buyer checks out as a guest
CREATE TABLE orders (
  id           BIGINT UNSIGNED                               NOT NULL AUTO_INCREMENT,
  ...
  -- "registered user OR guest" validation is handled in the service layer (Spring Boot)
  -- MySQL does not allow CHECK constraints on columns used in foreign keys
  CONSTRAINT chk_orders_total_nonneg CHECK (total >= 0 AND subtotal >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Order line items
-- Title and price are saved as a snapshot so changes to the product don't affect past orders
...
-- Payment linked 1:1 to an order
